...
hdcm = datacursormode(h);
datacursormode on;
set(hdcm, 'updatefcn', @get_alpha);
set(gcf, 'UserData', hdcm); % Pretty annoying.
...
% 	c = tikhonov(U, S, D, c, alpha);
	...
	mph = plot(out.t, out.f{i}, 'LineWidth', 1, 'Color', bc, ...
		'LineSmoothing', 'on');
	
	set(gca, 'XScale', xscale);
	set(gca, 'YScale', yscale);
	
	set(mph, 'UserData', 'PlotData');
	...
	eplh = plot(out.eta(1:i), out.rho(1:i), 'o:', ...
...
function output_txt = get_alpha(~, event_obj)
% Display position and alpha on the corrrect plots.
%
%

cl = get(event_obj, 'Target');
pos = get(event_obj, 'Position');

ud = get(cl, 'UserData');
hdcm = datacursormode(gcf);

h = handle(get(hdcm, 'DataCursors'));
hdc = get(h, 'DataCursorHandle');

ii = get(hdc, 'DataIndex');
...
if ~isempty(ud) && isstruct(ud)
	alphas = ud.alpha;
	output_txt = [output_txt, {['a: ', num2str(alphas(ii))], ...
		['log10(a): ', num2str(log10(alphas(ii)))]}];
	
	
	hls = findobj('type', 'line');
	
	for jj = 1:length(hls)
		udjj = get(hls(jj), 'UserData');
		if ~isempty(udjj) && strcmp(udjj, 'PlotData')
				set(hls(jj), 'YData', ud.f{ii});
				break;
		end
	end
